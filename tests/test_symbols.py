...
from iampypsa.formats.gams import load_indexed
from iampypsa.formats.iamc import load_variables
from iampypsa.quantities import (
    load_quantity,
    load_quantity_specs,
    load_simple,
...
class _FakeLoader:
    ...
    def __init__(self, frames: dict[str, pd.DataFrame], backend: str = "gdx"):
        self.frames = frames
    ...
    def read(self, ref, rename_columns=None):
        return self.frames[self.resolve(ref)].copy()


def test_load_specs_default():
    default = load_quantity_specs(backend="gdx")
...
def test_merge_region_overrides_prefers_region_entry():
    ...
    from iampypsa.quantities import merge_region_overrides
...
def test_load_specs_region_without_override_equals_default():
    assert load_quantity_specs("DEU", backend="gdx") == load_quantity_specs(backend="gdx")
...
def test_overlay_path_layers_onto_package_default(tmp_path):
    ...
    specs = load_quantity_specs(path=p, backend="gdx")
    ...
    cha = load_quantity_specs("CHA", path=p, backend="gdx")
    assert cha["co2_price"]["symbol"] == "cha_symbol"  # overlay region override wins


def test_default_config_path_exists():
    from iampypsa.models import get_default_config_path

    assert get_default_config_path("remind", "gdx").name == "quantities_gdx.yaml"
    assert get_default_config_path("remind", "iamc").name == "quantities_mif.yaml"
    assert "default:" in get_default_config_path("remind", "gdx").read_text()


def test_default_config_path_requires_a_known_backend():
    ...
    with pytest.raises(ValueError, match="no packaged config"):
        get_default_config_path("remind", "mif")
    with pytest.raises(TypeError):
        load_quantity_specs()  # backend is keyword-only and required
...
def test_load_frame_no_conversion_without_to_unit():
    ...
    out = load_simple(loader, {"symbol": "sym", "unit": "TUSD/TWa"})  # no to_unit → untouched
...
def test_load_frame_no_unit_column_when_unit_undeclared():
    loader = _FakeLoader({"sym": pd.DataFrame({"value": [5.0]})})
    out = load_simple(loader, {"symbol": "sym"})  # no unit/to_unit declared at all
...
def test_load_frame_uses_live_unit_when_declared_unit_matches():
    ...
    out = load_simple(loader, {"symbol": "sym", "unit": "GW", "to_unit": "MW"})
...
def test_load_frame_raises_on_declared_unit_mismatch():
    ...
    with pytest.raises(ValueError, match="does not match"):
        load_simple(loader, {"symbol": "sym", "unit": "TUSD/TWa", "to_unit": "MW"})
...
def test_load_set_splits_mixed_units_via_schema():
    ...
    out = load_indexed(loader, spec).set_index("parameter")
...
def test_load_symbol_specs_iamc_backend():
    iamc = load_quantity_specs(backend="iamc")
    ...
    gdx = load_quantity_specs(backend="gdx")
...
def test_load_variable_set_basic(tmp_path):
    ...
    result = load_variables(loader, spec)
...
def test_load_spec_dispatches_on_shape(tmp_path):
    ...
    result = load_quantity(loader, var_set_spec)
    assert len(result) == 1
    assert result["value"].iloc[0] == pytest.approx(1500.0)


def test_variables_shape_rejects_a_gdx_backend():
    # Dispatch is on spec shape, but a shape only one data model can serve must fail loudly
    # rather than fall through to a loader that would ignore the keys it can't honour.
    loader = _FakeLoader({}, backend="gdx")
    var_set_spec = {"variables": {"Cap|Electricity|Gas|GT": "gas-ocgt"}, "label_col": "technology", "to_unit": "MW"}
    with pytest.raises(ValueError, match=r"'variables' spec needs a \['iamc'\] loader"):
        load_quantity(loader, var_set_spec)


def test_indexed_shape_rejects_an_iamc_backend():
    """The mirror case, which used to pass silently through to the single-quantity loader."""
    loader = _FakeLoader({}, backend="iamc")
    spec = {"symbol": "pm_data", "index": "char", "schema": {"lifetime": {"parameter": "lifetime"}}}
    with pytest.raises(ValueError, match=r"'indexed' spec needs a \['gdx'\] loader"):
        load_quantity(loader, spec)
...
def _mif_canonical_names() -> set[str]:
    ...
    mif = read_quantity_config(backend="iamc")["default"]
    names: set[str] = set()
...
def test_mif_vocabulary_matches_gdx_technology_names():
    """quantities_mif.yaml's canonical names must be exactly the values gdx tokens map to."""
    gdx = read_quantity_config(backend="gdx")["default"]
...
def test_load_frame_against_real_gdx():
    ...
    spec = load_quantity_specs(backend="gdx")["co2_price"]
...
def test_load_frame_against_real_mif_uses_live_unit():
    ...
    loader = IamLoader(str(GENERIC_MIF))
    spec = dict(load_quantity_specs(backend="iamc")["hydro_capacity"])
    assert spec["unit"] == "GW"  # matches the live mif value -- documentation is accurate
    live_df = load_simple(loader, spec)
    assert (live_df["unit"] == "MW").all()

    spec["unit"] = "kW"  # now stale/wrong
    with pytest.raises(ValueError, match="does not match"):
        load_simple(loader, spec)
...
def test_demand_energy_balance_variable_set_converts_ej_to_mwh():
    ...
    loader = IamLoader(str(GENERIC_MIF))
    spec = load_quantity_specs(backend="iamc")["demand_energy_balance"]
    df = load_variables(loader, spec)
...
def test_demand_electrolysis_efficiency_converts_percent_to_pu():
    ...
    loader = IamLoader(str(GENERIC_MIF))
    spec = load_quantity_specs(backend="iamc")["demand_electrolysis_efficiency"]
    df = load_simple(loader, spec)
...
def test_build_regional_demand_matches_hand_computed_electrolysis():
    ...
    from iampypsa import IamLoader
    from iampypsa.models.remind import RemindIamcCoupler
    from iampypsa.units import unit_factor

    loader = IamLoader(str(GENERIC_MIF))
    quantities = load_quantity_specs(backend="iamc")

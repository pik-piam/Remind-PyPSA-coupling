"""Disaggregate REMIND regional demand to country level (region→country, Stage 1).

Ported from PyPSA-Eur ``downscale_REMIND_demand._disaggregate``. Single-member regions are
a no-op; multi-member regions are split by SSP GDP/population shares. Demand attributed to
unconfigured countries is dropped (with a warning above 1% of regional demand).
"""
...
from rpycpl.downscale.proxy import build_proxy_shares
...
def disaggregate_demand_to_country(
    ...
    for _, row in sectoral_load.iterrows():
        ...
        if not members:
            logger.warning("REMIND region '%s' not in region mapping — skipping.", region)
            continue
        ...
        if unconfigured and region not in warned:
            ...
            logger.warning(
                "REMIND region '%s' has unconfigured countries %s (%.1f%% of demand) — excluded.",

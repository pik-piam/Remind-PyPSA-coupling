...
Schema is documented in the model's technology_mapping_REMIND.yaml header. An ``IAM``-declared
...
def load_technology_parameters(path: str | PathLike) -> dict[str, Any]:
    """Return ``{"technologies": {name: spec}}`` from the model's technology_mapping_REMIND.yaml."""
    with open(path) as f:
        return {"technologies": yaml.safe_load(f)}


def iam_name(tech: str, spec: Any) -> str:
    """Return the REMIND/IAM technology name an entry pulls IAM values from."""

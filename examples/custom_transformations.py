""" it is straighforward to add your own transformations using the @register_etl decorator"""
...
@register_etl("subtract_h2") # registry name
def subtract_h2_from_el(loads: dict[str, pd.DataFrame], region: str = None) -> pd.DataFrame:
    """custom transformation to subtract the h2 load from the ac load (example)
    Args:
    ...
    return loads["ac_load"].copy() - loads["h2_el_load"].copy()
